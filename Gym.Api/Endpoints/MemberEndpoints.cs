...
public static class MemberEndpoints
{
    public static RouteGroupBuilder MapMemberEndpoints(this RouteGroupBuilder g)
    {
        ...
        group.MapPost("", [Authorize(Roles = "DATA_ENTRY,ADMIN")]async (MemberDto dto, IMemberService svc) =>
            Results.Created("/api/members", await svc.Add(dto)));
        group.MapPut("/{id:long}", [Authorize(Roles = "DATA_ENTRY,ADMIN")] async (long id, MemberDto dto, IMemberService svc) =>
        {
            if (id != dto.Id) return Results.BadRequest();
            var updated = await svc.Update(dto);
            return updated is null ? Results.NotFound() : Results.Ok(updated);
        });
        group.MapDelete("/{id:long}", [Authorize(Roles = "ADMIN")] async (long id, IMemberService svc) =>
